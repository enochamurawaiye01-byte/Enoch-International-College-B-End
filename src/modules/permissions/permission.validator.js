...
const createPermissionSchema = z.object({ key: z.string().trim().min(3).max(120).regex(/^[a-z0-9-]+\.[a-z_]+$/), module: z.string().trim().min(2).max(60), action, description: z.string().trim().max(300).nullable().optional() }).strict();
