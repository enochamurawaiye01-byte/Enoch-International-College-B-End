...
const convertToStudent = async (id, data) => {
	...
	const email = data.email || admission.email || null;
	...
	const desiredClassId = data.currentClassId || admission.desiredClassId || null;
	...
	return prisma.$transaction(async (tx) => {
		...
		const user = await tx.user.create({ data: { schoolId: data.schoolId || null, fullName, email: email?.toLowerCase(), phoneNumber: data.phoneNumber || admission.phoneNumber || null, passwordHash: await hashPassword(data.password || `${admission.firstName}123!`), role: "STUDENT", status: "ACTIVE" } });
		const registrationNumber = await generateRegistrationNumber(tx, fullName);
		const student = await tx.student.create({ data: { userId: user.id, registrationNumber, admissionNumber: admission.applicationNumber, firstName: admission.firstName, middleName: admission.middleName, lastName: admission.lastName, dateOfBirth: admission.dateOfBirth, gender: admission.gender, address: admission.address, currentClassId: desiredClassId, status: "ACTIVE", admissionDate: new Date() }, include: { currentClass: true } });
