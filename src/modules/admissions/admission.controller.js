...
const update = async (req, res, next) => { try { return res.json({ success: true, data: await service.update(req.params.id, req.body, req.user.id) }); } catch (error) { next(error); } };
const convertToStudent = async (req, res, next) => { try { return res.status(201).json({ success: true, data: await service.convertToStudent(req.params.id, { ...req.body, schoolId: req.user.schoolId }) }); } catch (error) { next(error); } };
