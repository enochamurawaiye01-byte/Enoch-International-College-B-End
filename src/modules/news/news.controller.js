...
const create = async (req, res, next) => { try { const media = req.file ? await uploadFile({ file: req.file, folder: "news" }) : null; const data = { ...req.body, ...(media ? { featuredImageUrl: media.url } : {}) }; return res.status(201).json({ success: true, data: await service.create(data, req.user) }); } catch (error) { next(error); } };
const list = async (req, res, next) => { try { return res.json({ success: true, data: await service.list(req.query) }); } catch (error) { next(error); } };
const getById = async (req, res, next) => { try { return res.json({ success: true, data: await service.getById(req.params.id, Boolean(req.user)) }); } catch (error) { next(error); } };
